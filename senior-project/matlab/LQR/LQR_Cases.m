...
% ===================== MODIFIED VERSION =====================
% Added:
%   (1) Second-order actuator lag on commanded joint accelerations qdd_cmd
%   (2) Torque saturation on tau (per-joint limits)
% Similar in spirit to PID actuator bandwidth modeling
% ============================================================
...
%% ===== NEW: ACTUATOR 2nd-ORDER LAG + TORQUE SATURATION (EDIT HERE IF NEEDED) =====
% Actuator model: qdd_real(s) = (wa^2)/(s^2 + 2*za*wa*s + wa^2) * qdd_cmd(s)
P.wa = 8;        % rad/s  (increase -> "faster" actuators, less lag)
P.za = 0.8;      % damping ratio

% Per-joint torque saturation (N·m). Choose realistic numbers for your setup.
% If your previous plots show ~14 N·m max, you can set slightly above/below
% depending on how "hard" you want to constrain.
P.tau_max = [14; 14; 14];   % [tau1_max; tau2_max; tau3_max]
...
for i = 1:nCases
    ...
    fprintf('Actuator: wa=%.2f rad/s, za=%.2f | Torque sat: [%g %g %g] N.m\n', ...
        P.wa, P.za, P.tau_max(1), P.tau_max(2), P.tau_max(3));
