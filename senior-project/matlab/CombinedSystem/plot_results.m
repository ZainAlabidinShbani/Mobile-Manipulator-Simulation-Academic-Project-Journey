function plot_results(t, log, obstacles, world)
figure('Name','Base path + obstacles');
plot(log.base(:,1), log.base(:,2), 'LineWidth',2); hold on; grid on;
plot(log.base_d(:,1), log.base_d(:,2), '--', 'LineWidth',1.2);
for j=1:size(obstacles,1)
    draw_circle(gca, obstacles(j,1), obstacles(j,2), obstacles(j,3));
end
axis equal; xlim(world.xlim); ylim(world.ylim);
xlabel('x'); ylabel('y'); legend('base','goal'); title('Base trajectory');

figure('Name','Phases');
plot(t, log.phase, 'LineWidth',1.3); grid on; xlabel('t [s]'); ylabel('phase');

figure('Name','EE tracking (world)');
subplot(3,1,1); plot(t,log.ee(:,1), t,log.ee_d(:,1),'--','LineWidth',1.1); grid on; ylabel('x');
subplot(3,1,2); plot(t,log.ee(:,2), t,log.ee_d(:,2),'--','LineWidth',1.1); grid on; ylabel('y');
subplot(3,1,3); plot(t,log.ee(:,3), t,log.ee_d(:,3),'--','LineWidth',1.1); grid on; ylabel('z'); xlabel('t [s]');
end
