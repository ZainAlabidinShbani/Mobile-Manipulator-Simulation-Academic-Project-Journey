function animate_case(P, log, p_pick, p_place, stepAnim, gifFile, gifDelay, titlePrefix)

fig = figure('Color','w','Renderer','opengl');
axes('Parent',fig); %#ok<NASGU>
hold on; grid on; axis equal;
xlabel('X (m)'); ylabel('Y (m)'); zlabel('Z (m)');
view(45,25);
title(sprintf('%s | Actual (red) vs Desired (black)', titlePrefix),'Interpreter','none');
...
h_ee_act = plot3(0,0,0,'ro','MarkerSize',7,'MarkerFaceColor','r');
h_ee_des = plot3(0,0,0,'ko','MarkerSize',6,'MarkerFaceColor','k');
...
for k = 1:stepAnim:length(log.t)
    ...
    set(h_ee_act,'XData',p3(1),'YData',p3(2),'ZData',p3(3));
    set(h_ee_des,'XData',pd(1),'YData',pd(2),'ZData',pd(3));
    ...
    Xd(end+1)=pd(1); Yd(end+1)=pd(2); Zd(end+1)=pd(3);
    ...
    try
        ...
        tmpPng = [tempname,'.png'];
        exportgraphics(fig, tmpPng, 'Resolution', 120);
        img = imread(tmpPng);
        delete(tmpPng);
    end
