...
import os
from ament_python import get_package_share_directory  # noqa (alias below)
...
def generate_launch_description():
    ...
    declare_slam = DeclareLaunchArgument(
        ...
        description='Run SLAM Toolbox for mapping (true) or use saved map (false)'
    )
    declare_map = DeclareLaunchArgument(
        'map', default_value='',
        description='Path to saved map YAML (used when slam:=false)'
    )
    ...
    declare_auto_nav = DeclareLaunchArgument(
        ...
        description='Automatically navigate when YOLO detects target'
    )
    declare_target = DeclareLaunchArgument(
        'target_class', default_value='cube',
        description='YOLO class name to trigger navigation'
    )
